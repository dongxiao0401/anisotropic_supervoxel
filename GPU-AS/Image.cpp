...
/*
 init seeds in grid at the begining
*/
void Image::init_seeds()
{
    //1. init grid seeds
    //------------------
    double voxel_size = texel_num / seeds_num;
    if (dbname == "MiddleburyFlow")
    {
        //MiddleBuryFlow dataset only constains 2 frames
        s = int(floor(pow(1.0*width*height / seeds_num, 0.5)));
    }
    else
    {
    ...
    stepH = height / hNum;
    stepW = width / wNum;
    //max_s: the max step in 3 dimensions
    max_s = stepD;
    ...
    cout << "seed num placed:" << count << endl;

    glGenVertexArrays(1, &VAO_s);
    glGenBuffers(1, &VBO_s);
    glBindVertexArray(VAO_s);
    glBindBuffer(GL_ARRAY_BUFFER, VBO_s);
    glBufferData(GL_ARRAY_BUFFER, seeds_lsc.size() * sizeof(float), &seeds_lsc[0], GL_STATIC_DRAW);//sizeof(pos)Ö»ÄÜµ±posÊ±Êý×éÊ±²ÅÄÜÕâÑùÓÃ¡£±ÈÈçfloat pos[]={1,1,1};,ÕâÑùµÄ»°sizeof(pos)=3*sizeof(float)
    ...
    //2. opengl VertexArray
    //seeds_index: tex_x,tex_y,tex_z,id. Wirte to the first k pixels of a texture.
    vector<float> seeds_index;
    ...
    glBufferData(GL_ARRAY_BUFFER, seeds_index.size() * sizeof(float), &seeds_index[0], GL_STATIC_DRAW);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 4 * sizeof(float), (void*)0);
    glEnableVertexAttribArray(0);
    glVertexAttribPointer(1, 1, GL_FLOAT, GL_FALSE, 4 * sizeof(float), (void*)(3 * sizeof(float)));
    glEnableVertexAttribArray(1);

    // all_texels: x,y,z. Wirte to all pixels of a texture.
...
bool Image::init_flood_texture()
{
    ...
    /*glBindTexture(GL_TEXTURE_3D, flood_texture[0]);
    glDrawBuffers(2, flood_texture);
...
bool Image::init_image_texture(unsigned char* data)
{
    ...
    for (unsigned int i = 1; i < 2; ++i)
    {
        ...
        glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
        //GL_HALF_FLOAT£º6.10 * 10^-5 to 65504
...
{
    unsigned char* res1 = new unsigned char[texel_num * 4];//GL_RGB,unsigned char
    glGetTexImage(GL_TEXTURE_3D, 0, GL_RGB, GL_UNSIGNED_BYTE, res1);
    char* txtname = (char*)"rgb_3d.txt";
    std::ofstream f_debug1(txtname);
    int pi = 0;
...
/*
prepare GPU texture, initialize seeds as grids
*/
bool Image::init_gpu(string& dbname_, unsigned char* data, int width_, int height_, int depth_, int seeds_num_, int Lloyd_max_iter_, float compactness_)
{
    dbname = dbname_;
    width = width_;
    height = height_;
...
/*
Adjust seed initialization based on object richness
*/
void Image::initialization(Shader& derivative_t, Shader& density_grid)
{
    //The grid of seed in 3D
    ...
    /*glBindTexture(GL_TEXTURE_3D, texture_image[2]);
    print_image_label_float();*/
    if (grid) delete[] grid;

    //calculate the derivative of pixels on x,y,t dimensions
    derivative_t.use();
    glBindFramebuffer(GL_FRAMEBUFFER, frameBuffer_image);
    glDrawBuffers(1, buffers_image + 1);
    glClearColor(3.0f, -1.0f, -1.0f, -1.0f);
    ...
    //calculate the sum derivative info of each grid.
    glEnable(GL_BLEND);
    ...
    seeds_lsc_adjust.clear();
    float adjust_x = 0;//only adjust x position
    int backforth = 1;
    ...
    for (int i = 0; i < number_one_layer; )
    {
        if (seed_in[i] < seed_length[j1])
        {
            ...
            seeds_lsc_adjust.push_back(adjust_y);//seeds_lsc[j1 * 4 + 1]
...
void Image::print_flow_mat(Mat& flow)
{
    ...
    for (int i = 0; i < row; i++) {
        ...
        for (int j = 0; j < col; j++) {
            f_debug << std::left << std::setw(4) << ptr[j] << " ";
        }
        f_debug << endl;
    }
...
void Image::forward_backward_flow(vector<GpuMat>& gpu_frames, vector<Mat>& flow_forward, vector<Mat>& flow_backward)
{
    GpuMat gpu_flow;
    ...
    //cuda::DensePyrLKOpticalFlow::create();
...
void Image::seed_matrix(vector<Mat>& flow_forward, vector<Mat>& flow_backward)
{
    //calculate seed matrix based on optical flow
    seeds_lsc.clear();
    ...
    for (int i = 0; i < ac_seeds_num; i++)
    {
        ...
        //save matrix value if it is PSD
        if (!neg_eigen_value)
        {
            for (int t = 0; t < 6; t++)
            {
            ...
            //if matrix is not PSD, calculate neighbors' matrix, and take the average of neighboring PSD matrices as current matrix
        else
        {
            neg_seed_matrix_num++;
            //cout << "seed " << i<<": ";
            ...
            if (valid_ng_matrix_num == 0)
            {
                ...
                //if matrix is not PSD, and neighbors cann't help, find its nearest PSD.
                if (valid_ng_matrix_num == 0)
                {
    ...
    //cout << "!!!non-PSD martrix num:" << neg_seed_matrix_num << endl;

    //seed info: x,y,z,id,symmetric matrix 6 unknowns are saved in seeds_lsc£¬then to texture
    glBindVertexArray(VAO_s);
    glBindBuffer(GL_ARRAY_BUFFER, VBO_s);
...
bool Image::matrix_slover(float seed_x, float seed_y, int on_frame_index, vector<Mat>& flow_forward, vector<Mat>& flow_backward, vector<float>& matrix_6_paras)
{
    ...
    /*cout << "matrix_A_vec" << endl;
    for (int t = 0; t < matrix_A_vec.size(); t++)
    {
    cout << matrix_A_vec[t] << ",";
    if (t % 6 == 5)
    cout << endl;
    ...
    /*seeds_lsc.push_back(m_m[0]);
    seeds_lsc.push_back(m_m[1]);
    seeds_lsc.push_back(m_m[2]);
    seeds_lsc.push_back(m_m[3]);
    seeds_lsc.push_back(m_m[4]);
    seeds_lsc.push_back(m_m[5]);*/
...
/*
 draw seeds information to textures
*/
void Image::Ld_first_init(Shader& draw_seeds)
{
    //texture_image[0,3,2,4]: seed coordinates, seed color, seed matrix first 3 parameters, seed matrix last 3 parameters
    glBindFramebuffer(GL_FRAMEBUFFER, frameBuffer_flood);
    draw_seeds.use();
...
void Image::init_variables()
{
    //control the compactness of supervoxel
    compactness = 8;
...
void Image::print_seg(unsigned char* data, int* labels)
{
    int index = 0;
...
void Image::centroids_next_seeds(Shader& centroids_next_seeds, Shader& aver_exception, int Lloyd_curr_iter)
{
    ...
    glEnable(GL_BLEND);
    glBlendFunc(GL_ONE, GL_ONE);
    glBlendEquation(GL_FUNC_ADD);//GL_MIN,GL_MAX
    ...
    centroids_next_seeds.setFloat("ac_seeds_num", float(ac_seeds_num));//ÒòÎªlabelÓÐ¿ÉÄÜÎª-1£¬ËùÒÔÍ³ŒÆÒ»ÏÂÀïÃæÊÇ·ñÓÐ-1µÄÏñËØ£¬ÓÐµÄ»°ÐèÒªºóÐøŽŠÀí
    glBindVertexArray(VAO_all_t);
    glDrawArrays(GL_POINTS, 0, texel_num);
...
void Image::Ld_next_seeds_matrix(Shader& draw_next_seeds, vector<Mat>& flow_forward, vector<Mat>& flow_backward, int Lloyd_curr_iter)
{
    ...
    int read_height = (ac_seeds_num / width) + 1;
    //int read_width = ac_seeds_num % width;
    glBindFramebuffer(GL_FRAMEBUFFER, frameBuffer_flood);
    glBindTexture(GL_TEXTURE_3D, texture_flood[1 - current_buffer]);
    //print_buffer(1-current_buffer);
    ...
    /*ofstream f_debug("seed_pos.txt");
    int id = 0;
    for (int i = 0; i < read_height; i++)
    {
    for (int j = 0; j < width; j++)
    {
    f_debug << res_pos[id * 4] << " " << res_pos[id * 4 + 1] << " " << res_pos[id * 4 + 2] << " " << res_pos[id * 4 + 3] << " ";
    id++;
    }
    }
    f_debug.close();*/
    ...
    seeds_lsc.clear();
    seeds_lsc.reserve(ac_seeds_num * 13);//x,y,z,id,l,a,b,6 unknowns of symmetric matrix 

    int on_frame_index = 0;
    float seed_x = 0, seed_y = 0, seed_z = 0;
    Vec2f seed_flow, neighbor_flow;
    int ng_x = 0, ng_y = 0;
    int index = 0;
    vector<float> curr_matrix;
    vector<float> self_matrix;
    int neg_seed_matrix_num = 0;
    vector<int> neg_seed_vec;
    /*int neg_seed_matrix_num_2 = 0;
    vector<int> neg_seed_vec_2;*/
    int psd_seed_matrix_num = 0;
    vector<int> psd_seed_vec;
    ...
    for (int i = 0; i < ac_seeds_num; i++)
    {
        ...
        seed_x = floor(seed_x);
        seed_y = floor(seed_y);
        seed_flow = flow_forward[on_frame_index].at<Vec2f>(int(seed_y), int(seed_x));
        ...
        else
        {
            ...
            for (int t = 0; t < 8; t++)
            {
                ...
                if (seed_ng_x >= 0 && seed_ng_x < width&&seed_ng_y >= 0 && seed_ng_y < height)
                {
                    neighbor_flow = flow_forward[on_frame_index].at<Vec2f>(int(seed_ng_y), int(seed_ng_x));
                    float cos = (seed_flow[0] * neighbor_flow[0] + seed_flow[1] * neighbor_flow[1]) / (sqrt(pow(seed_flow[0], 2) + pow(seed_flow[1], 2)) * sqrt(pow(neighbor_flow[0], 2) + pow(neighbor_flow[1], 2)));
                    float theta = acos(cos)*(180 / PI);
                    if (theta < 90)
                    {
                        neg_eigen_value = matrix_slover(seed_ng_x, seed_ng_y, on_frame_index, flow_forward, flow_backward, curr_matrix);
                        if (!neg_eigen_value)
            ...
            if (valid_ng_matrix_num == 0)
            {
                ...
                if (valid_ng_matrix_num == 0)
                {
                    ...
                    /*seeds_lsc.push_back(1);
                    seeds_lsc.push_back(0);
                    seeds_lsc.push_back(0);
                    seeds_lsc.push_back(1);
                    seeds_lsc.push_back(0);
                    seeds_lsc.push_back(1);*/
...
void Image::check_dead_seeds(Shader& label, Shader& label_replace, int* labels)
{
    glBindFramebuffer(GL_FRAMEBUFFER, frameBuffer_flood);
    glBindTexture(GL_TEXTURE_3D, texture_flood[current_buffer]);
    //print_buffer(current_buffer);
    //read seeds
    int read_height = ((ac_seeds_num + 1) / width) + 1;//ac_seeds_num+1£¬the last one is the number of label '-1'
    ...
    if (res1[ac_seeds_num * 4 + 3] > 0)
    {
        ...
        cout << "!!!-1 label exist,num£º" << res1[ac_seeds_num * 4 + 3] << endl;
    }
    ...
    {
        //label discontinuity: dead_seed_num <= 3, replace last label with the missing label
...
void Image::sequence_label(int* labels, map<int, int>& sequence_label_map, bool dead_seed)
{
    ...
    {
        //label invalid: some labels are '-1', because they did not receive any seed info.
...
void Image::print_sv_txt(int* labels, string dirName, bool output_label, bool connected)
{
    if (output_label)
    {
        ...
        for (int t = 0; t < depth; t++)
        {
            for (int i = height - 1; i >= 0; i--) {
                ...
                for (int j = 0; j < width; j++) {
                    f_debug1 << std::left << std::setw(4) << int(labels[slice_index]) << " ";
                    slice_index++;
                }
                f_debug1 << endl;
            }
        ...
        f_debug1.close();
		if(connected)
			cout << "save connected label!!!" << endl;
		else
			cout << "save not connected label!!!" << endl;
    }
}

void Image::enforce_connectivity_3d(Shader& label, int* labels, int& label_num)
{
    glBindFramebuffer(GL_FRAMEBUFFER, frameBuffer_image);
    label.use();
    glDrawBuffers(1, buffers_image + 3);
    ...
    //relabel in 3D, merge small clusters
...
void Image::print_render(int* labels)
{
    ...
    for (int i = 0; i < depth; i++)
    {
        ...
        cvtColor(seg, seg, COLOR_BGR2RGB);
        flip(seg, seg, 0);//rows-i-1,j
        imshow("Supervoxels", seg);
        waitKey();

        /*	char savepath[1024];
            sprintf_s(savepath, "voxel\\MiddleburyFlow\\%05d.png", i + 1);
        ...
        for (int j = 0; j < height; j++)
        {
            for (int k = 0; k < width; k++)
            {
                if (0 > nlabels[oindex])
                {
                    ...
                    //-------------------------------------------------------
                    if (count < (SUPSZ / 4))//>> 2±íÊŸ³ýÒÔ4,SUPSZ >> 3
                    {
                        for (int c = 0; c < count; c++)
                        {
...
void Image::print_image_label()
{
    ...
    char* txtname_=(char*)txtname.data();
    ofstream f_debug1(txtname_);
    int slice_index = 0;
    for (int t = 0; t < depth; t++)
    {
...
void Image::print_final(int index)
{
    ...
    string txtname = "rgb_flood" + to_string(index) + ".txt";
    char* txtname_=(char*)txtname.data();
    ofstream f_debug1(txtname_);
    int slice_index = 3;
    for (int t = 0; t < depth; t++)
...
void Image::print_buffer(int index)
{
    ...
    */
    ...
    float* res1 = new float[texel_num * 4];//GL_RGBA
    glGetTexImage(GL_TEXTURE_3D, 0, GL_RGBA, GL_FLOAT, res1);
    string txtname = "rgb" + to_string(index) + ".txt";
    char* txtname_=(char*)txtname.data();
    ofstream f_debug1(txtname_);
    int slice_index = 0;
    for (int t = 0; t < depth; t++)
