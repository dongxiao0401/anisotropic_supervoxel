#pragma once
#ifndef OPENCV_OPENGL_TEST_IMAGE_H
#define OPENCV_OPENGL_TEST_IMAGE_H

#endif //OPENCV_OPENGL_TEST_IMAGE_H
...
public:
    ...
    void initialization(Shader& derivative_t, Shader& density_grid);
    bool init_gpu(string& dbname, unsigned char* data, int width, int height, int depth, int seeds_num, int Lloyd_max_iter,float compactness_);
	...
	void check_dead_seeds(Shader& label, Shader& label_replace, int* labels);
    void forward_backward_flow(vector<GpuMat>& gpu_frames, vector<Mat>& flow_forward, vector<Mat>& flow_backward);
    void seed_matrix(vector<Mat>& flow_forward, vector<Mat>& flow_backward);
    void enforce_connectivity_3d(Shader& label, int* labels, int& final_label_num);
    void print_sv_txt(int* labels, string dirName, bool output_label,bool connected);
    void print_connect_seg(unsigned char* data, int* labels);
	void print_seg(unsigned char* data, int* labels);
    void print_render(int* labels);
private:
